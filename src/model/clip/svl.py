...
class GlitchBlock(nn.Module):
    ...
    def forward(self, attrs):
        ...
        for attr in ["q", "k", "v"]:
            ...
            affs.append(self.t_convs[attr](aff))  # shape = (n*l, r, t, t) where r is number of filters

        aff = torch.cat(affs, dim=1)  # shape = (n*l, 3*r, t, t)

        aff = aff.unflatten(0, (b, p, p)).flatten(3)  # shape = (n, p, p, 3*r*t*t)
        aff = aff.permute(0, 3, 1, 2)  # shape = (n, 3*r*t*t, p, p)
